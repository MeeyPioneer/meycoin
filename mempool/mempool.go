...
import (
	...
	cfg "github.com/meeypioneer/meycoin/config"
	...
	"github.com/meeypioneer/meycoin/fee"
	"github.com/meeypioneer/meycoin/internal/common"
	"github.com/meeypioneer/meycoin/internal/enc"
...
func (mp *MemPool) getNameDest(account []byte, owner bool) []byte {
	...
	if string(account) == string(types.MeyCoinVault) {
		return account
	}

	nameState, err := mp.getAccountState([]byte(types.MeyCoinName))
	if err != nil {
		mp.Error().Str("for name", string(account)).Msgf("failed to get state %s", types.MeyCoinName)
		return nil
	}
	scs, err := mp.stateDB.OpenContractState(types.ToAccountID([]byte(types.MeyCoinName)), nameState)
	if err != nil {
		mp.Error().Str("for name", string(account)).Msgf("failed to open contract %s", types.MeyCoinName)
		return nil
	}
	if owner {
...
func (mp *MemPool) validateTx(tx types.Transaction, account types.Address) error {
	...
	case types.TxType_GOVERNANCE:
		meycoinState, err := mp.getAccountState(tx.GetBody().GetRecipient())
		if err != nil {
			return err
		}
		aid := types.ToAccountID(tx.GetBody().GetRecipient())
		scs, err := mp.stateDB.OpenContractState(aid, meycoinState)
		if err != nil {
			return err
		}
		...
		case types.MeyCoinName:
			systemcs, err := mp.stateDB.OpenContractStateAccount(types.ToAccountID([]byte(types.MeyCoinSystem)))
			if err != nil {
				return err
			}
			sender, err := mp.stateDB.GetAccountStateV(account)
		...
		case types.MeyCoinEnterprise:
			enterprisecs, err := mp.stateDB.OpenContractStateAccount(types.ToAccountID([]byte(types.MeyCoinEnterprise)))
			if err != nil {
				return err
			}
			sender, err := mp.stateDB.GetAccountStateV(account)
	...
	case types.TxType_FEEDELEGATION:
		...
		meycoinState, err := mp.getAccountState(recipient)
		if err != nil {
			return err
		}
		bal := meycoinState.GetBalanceBigInt()
