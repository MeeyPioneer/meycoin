...
import (
	...
	"github.com/meeypioneer/meycoin/consensus/impl/dpos/bp"
	"github.com/meeypioneer/meycoin/consensus/impl/dpos/slot"
...
func sendVotingReward(bState *state.BlockState, dummy []byte) error {
	...
	vaultID := types.ToAccountID([]byte(types.MeyCoinVault))
...
func InitVPR(sdb *state.StateDB) error {
	s, err := sdb.OpenContractStateAccount(types.ToAccountID([]byte(types.MeyCoinSystem)))
	if err != nil {
		return err
	}
