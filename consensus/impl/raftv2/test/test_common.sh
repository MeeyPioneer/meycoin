...
function existProcess() {
    ...
    local proc=$(ps  -ef|grep meycoinsvr | grep $port | awk '{print $2 }')
    if [ "$proc" = "" ]; then
        return "0"
    fi
...
function getHeight() {
    ...
    local _height_=$(meycoincli -p $port blockchain | jq .Height)
...
function getHash() {
	...
	echo "meycoincli -p $port getblock --number $height | jq .Hash"
    local hash=$(meycoincli -p $port getblock --number $height | jq .Hash)
...
function getLeaderOf() {
	...
	_leader_=$(meycoincli -p $myport blockchain | jq .ConsensusInfo.Status.Leader)
	_leader_=${_leader_//\"/}

	if [[ "$_leader_" == meycoin* ]]; then
		eval "$2=$_leader_"
	fi
}
...
function getRaftID() {
	...
	echo "meycoincli -p $_leaderport getconsensusinfo | jq $pattern"
	_raftID=`meycoincli -p $_leaderport getconsensusinfo | jq $pattern`
	ret=$?
	if [ "$_raftID" == "" ]; then
...
function getRaftState() {
	...
	local _raftState=$(meycoincli -p $leaderPort getconsensusinfo | jq $pattern)
...
function checkLeaderValid() {
	...
	for i in $ALLIPS ; do
		...
		curleader=$(meycoincli -p $i blockchain | jq .ConsensusInfo.Status.Leader)
		curleader=${curleader//\"/}
		if [[ "$curleader" != meycoin* ]]; then
...
function getClusterTotal() {
	_chkPort=$1
	_total=`meycoincli -p $_chkPort blockchain | jq .ConsensusInfo.Status.Total`
...
function waitClusterTotal() {
	...
	for ((i=1; i<= $tryCnt ; i++)); do
		...
		total=`meycoincli -p $leaderport blockchain | jq .ConsensusInfo.Status.Total`
...
function WaitPeerConnect() {
	...
	for ((i = 1; i<= $_timeout; i++)); do
		_res=$(meycoincli -p 10001 getpeers | jq ".|length")
