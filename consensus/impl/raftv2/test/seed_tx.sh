...
# 지갑 생성 & 언락
meycoincli -p ${port} account unlock --address ${genesis_addr} --password 1234

chain_id_hash=`meycoincli  -p ${port} blockchain | jq .ChainIdHash`
...
while read line
do
	meycoincli -p ${port} signtx --jsontx \
...
# 트랜잭션 컨펌
meycoincli -p ${port} committx --jsontxpath $genesis_addr.tmp
