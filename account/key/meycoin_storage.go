...
type MeyCoinStorage struct {
	sync.RWMutex
	storePath string
}

func NewMeyCoinStorage(storePath string) (*MeyCoinStorage, error) {
	...
	return &MeyCoinStorage{
		storePath: absPath,
	}, nil
}

func (ks *MeyCoinStorage) Save(identity Identity, passphrase string, key *PrivateKey) (Identity, error) {
	ks.RWMutex.RLock()
...
func (ks *MeyCoinStorage) Load(identity Identity, passphrase string) (*PrivateKey, error) {
	// FIXME: save itself. need to refactor store.go
	encodedIdentity := types.EncodeAddress(identity)
...
func (ks *MeyCoinStorage) List() ([]Identity, error) {
	ks.RWMutex.RLock()
	defer ks.RWMutex.RUnlock()
