...
func Test_wezenListManager_saveListFile(t *testing.T) {
	tmpAuthDir, err := ioutil.TempDir("","meycoinTestWezen")
	if err != nil {
	...
	logger := log.NewLogger("wezen.test")
	conf := config.WezenConfig{EnableBlacklist: true}
	lm := NewWezenListManager(&conf, tmpAuthDir, logger)
	lm.entries = sampleEntries
	lm.saveListFile()

	lm2 := NewWezenListManager(&conf, tmpAuthDir, logger)
	lm2.loadListFile()
	if len(lm2.entries) != len(lm.entries) {
		t.Errorf("wezenListManager.loadListFile() entry count %v, want %v", len(lm2.entries), len(lm.entries))
	}
	...
	for i, e := range lm.entries {
		...
		if !reflect.DeepEqual(e, e2) {
			t.Errorf("wezenListManager.loadListFile() entry %v, %v", e, e2)
		}
	}
}

func Test_wezenListManager_RemoveEntry(t *testing.T) {
	logger := log.NewLogger("wezen.test")
	conf := &config.WezenConfig{EnableBlacklist: true}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			lm := NewWezenListManager(conf, "temp", logger)
			lm.entries = sampleEntries
...
func Test_wezenListManager_AddEntry(t *testing.T) {
	logger := log.NewLogger("wezen.test")
	conf := &config.WezenConfig{EnableBlacklist: true}
...
func Test_wezenListManager_IsBanned(t *testing.T) {
	conf := config.NewServerContext("", "").GetDefaultWezenConfig()
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			b := NewWezenListManager(conf, "/tmp", logger)
			b.entries = tt.preset
