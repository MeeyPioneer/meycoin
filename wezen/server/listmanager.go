...
func NewWezenListManager(conf *config.WezenConfig, authDir string, logger *log.Logger) *wezenListManager {
	lm := &wezenListManager{
		logger:  logger,
...
func (lm *wezenListManager) loadListFile() {
...
func (lm *wezenListManager) saveListFile() {
	blFile := filepath.Join(lm.authDir, localListFile)
...
func (lm *wezenListManager) ListEntries() []types.WhiteListEntry {
	lm.mutex.Lock()
	defer lm.mutex.Unlock()
...
func (lm *wezenListManager) AddEntry(entry types.WhiteListEntry) {
	lm.mutex.Lock()
	defer lm.mutex.Unlock()
...
func (lm *wezenListManager) RemoveEntry(idx int) bool {
	lm.mutex.Lock()
	defer lm.mutex.Unlock()
...
func (lm *wezenListManager) Start() {
...
func (lm *wezenListManager) IsBanned(addr string, pid types.PeerID) (bool, time.Time) {
	// wezen is blaklist
...
func (lm *wezenListManager) RefineList() {
	// no refine is needed
}

func (lm *wezenListManager) Summary() map[string]interface{} {
