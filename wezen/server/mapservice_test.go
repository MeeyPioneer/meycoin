...
func TestPeerMapService_BeforeStop(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			pms := NewWezenService(pmapDummyCfg, pmapDummyNTC)

			mockNT.EXPECT().AddStreamHandler(common.WezenMapSub, gomock.Any()).Times(1)
			mockNT.EXPECT().RemoveStreamHandler(common.WezenMapSub).Times(1)
...
func TestPeerMapService_handleQuery(t *testing.T) {
	minVersion, _ := p2pcommon.ParseMeyCoinVersion(p2pcommon.MinimumMeyCoinVersion)
	tooNewVersion, _ := p2pcommon.ParseMeyCoinVersion(p2pcommon.MaximumMeyCoinVersion)
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			mockNT.EXPECT().GetOrCreateStreamWithTTL(gomock.Any(), gomock.Any(), common.WezenPingSub).Return(mockStream, nil).MinTimes(1)
...
func TestNewMapService(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := NewWezenService(tt.args.cfg, tt.args.ntc); !reflect.DeepEqual(got, tt.want) {
				t.Errorf("NewWezenService() = %v, want %v", got, tt.want)
			}
		})
	}
}
...
func Test_isEqualMeta(t *testing.T) {
	...
	a3,_ := types.ParseMultiaddr("/dns4/test.meycoin.io/tcp/11002/p2p/16Uiu2HAmHuBgtnisgPLbujFvxPNZw3Qvpk3VLUwTzh5C67LAZSFh")
