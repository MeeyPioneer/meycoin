...
import (
	...
	"github.com/meeypioneer/meycoin/internal/network"
...
// internal
const (
	WezenPingTTL       = common.WezenConnectionTTL >> 1

	// wezen will return peers list at most this number
...
func NewWezenService(cfg *config.Config, ntc p2pcommon.NTContainer) *PeerMapService {
	pms := &PeerMapService{
		rwmutex:      &sync.RWMutex{},
		peerRegistry: make(map[types.PeerID]*peerState),
		allowPrivate: cfg.Wezen.AllowPrivate,
	}

	pms.BaseComponent = component.NewBaseComponent(WezenSvc, pms, log.NewLogger("wezen"))
	...
	pms.lm = NewWezenListManager(cfg.Wezen, cfg.BaseConfig.AuthDir, pms.Logger)
...
func (pms *PeerMapService) AfterStart() {
	...
	pms.lm.Start()
	pms.Logger.Info().Str("minMeyCoinVer", p2pcommon.MinimumMeyCoinVersion).Str("maxMeyCoinVer", p2pcommon.MaximumMeyCoinVersion).Str("version", string(common.WezenMapSub)).Msg("Starting wezen listening")
	pms.nt.AddStreamHandler(common.WezenMapSub, pms.onConnect)
	pms.hc.Start()
}
...
func (pms *PeerMapService) BeforeStop() {
	if pms.nt != nil {
		pms.hc.Stop()
		pms.nt.RemoveStreamHandler(common.WezenMapSub)
	}
	pms.lm.Stop()
}
...
func (pms *PeerMapService) handleQuery(conn p2pcommon.RemoteConn, container p2pcommon.Message, query *types.MapQuery) (*types.MapResponse, error) {
	...
	// for backward compatibility to meycoin v1 .
	...
	if query.AddMe && !query.Status.NoExpose {
		// check Sender
		// check peer is really capable to meycoinsvr
...
func createV030Message(msgID, orgID p2pcommon.MsgID, subProtocol p2pcommon.SubProtocol, innerMsg p2pcommon.MessageBody) (p2pcommon.Message, error) {
	...
	msg := common.NewWezenRespMessage(msgID, orgID, subProtocol, bytes)
	return msg, nil
}
...
func (pms *PeerMapService) onPing(s types.Stream) {
	peerID := s.Conn().RemotePeer()
	pms.Logger.Debug().Str(p2putil.LogPeerID, peerID.String()).Msg("Received ping from wezen (maybe)")
...
func (pms *PeerMapService) getCurrentPeers(param *CurrentListMsg) *types.WezenPeerList {
	retSize := int(param.Size)
	totalSize := len(pms.peerRegistry)
	listSize := calcMinimum(retSize, totalSize, ResponseMaxPeerLimit)
	pList := make([]*types.WezenPeer, listSize)
	addSize := 0
	pms.rwmutex.Lock()
	pms.rwmutex.Unlock()
	for _, rPeer := range pms.peerRegistry {
		pList[addSize] = &types.WezenPeer{Address: &rPeer.addr, Connected: rPeer.connected.UnixNano(), LastCheck: rPeer.lastCheck().UnixNano(), Verion:rPeer.meta.Version}
		addSize++
		if addSize >= listSize {
			break
		}
	...
	result := &types.WezenPeerList{Peers: pList, HasNext: false, Total: uint32(totalSize)}
	return result
}

func (pms *PeerMapService) getWhiteList(param *WhiteListMsg) *types.WezenPeerList {
	// TODO implement!
	return &types.WezenPeerList{}
}

func (pms *PeerMapService) getBlackList(param *BlackListMsg) *types.WezenPeerList {
	// TODO implement!
	return &types.WezenPeerList{}
}
...
func (pms *PeerMapService) checkConnectness(meta p2pcommon.PeerMeta) bool {
	if !pms.allowPrivate && !network.IsPublicAddr(meta.PrimaryAddress()) {
		pms.Logger.Debug().Str("peer_meta", p2putil.ShortMetaForm(meta)).Msg("peer is private address but wezen is not allow by configuration")
		return false
	}
	tempState := &peerState{PeerMapService: pms, meta: meta, addr: meta.ToPeerAddress(), lCheckTime: time.Now(), temporary: true}
	_, err := tempState.checkConnect(WezenPingTTL)
	if err != nil {
