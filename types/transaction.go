...
//governance type transaction which has meycoin.system in recipient
...
func InitGovernance(consensus string, isPublic bool) {
	...
	govValidators = map[string]validator{
		MeyCoinSystem: sysValidator,
		MeyCoinName:   validateNameTx,
		MeyCoinEnterprise: func(tx *TxBody) error {
			if isPublic {
...
func (tx *transaction) Validate(chainidhash []byte, isPublic bool) error {
	...
	if amount.Cmp(MaxMEY) > 0 {
		return ErrTxInvalidAmount
	}

	gasprice := tx.GetBody().GetGasPriceBigInt()
	if gasprice.Cmp(MaxMEY) > 0 {
