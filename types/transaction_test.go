...
func TestGovernanceTypeTransaction(t *testing.T) {
	...
	transaction.GetTx().Hash = transaction.CalculateTxHash()
	err = transaction.Validate(chainid, false)
	assert.EqualError(t, ErrTxInvalidRecipient, err.Error(), "recipient should be meycoin.*")

	transaction.GetTx().GetBody().Recipient = []byte(MeyCoinSystem)
	transaction.GetTx().Hash = transaction.CalculateTxHash()
	err = transaction.Validate(chainid, false)
	...
	transaction.GetTx().GetBody().Recipient = []byte(`meycoin.name`)
