...
const (
	...
	defaultMeyCoinConfigFileName = "config.toml"

	EnvironmentPrefix = "AG"

	//defaultLogFileName = "meycoin.log"
)
...
type Config struct {
	...
	Wezen    *WezenConfig    `mapstructure:"wezen"`
...
// BaseConfig defines base configurations for meycoin server
...
type P2PConfig struct {
	...
	NPDiscoverPeers bool     `mapstructure:"npdiscoverpeers" description:"Whether to discover from wezen or other nodes and connects"`
	...
	NPExposeSelf   bool     `mapstructure:"npexposeself" description:"Whether to request expose self to wezen and other connected node"`
	NPUseWezen   bool     `mapstructure:"npusewezen" description:"Whether to connect and get node list from wezen"`
	NPAddWezenes []string `mapstructure:"npaddwezenes" description:"Add addresses of wezenes if default wezen is not sufficient"`
...
// WezenConfig defines configuration for wezen server and client (i.e. wezenConnect)
type WezenConfig struct {
	AllowPrivate    bool   `mapstructure:"allowprivate" description:"allow peer to have private address. for private network and test"`
	GenesisFile     string `mapstructure:"genesisfile" description:"json file containing informations of genesisblock to which wezen refer "`
...
*/
const tomlConfigFileTemplate = `# meycoin TOML Configuration File (https://github.com/toml-lang/toml)
# base configurations
...
npexposeself = true
npusewezen = {{.P2P.NPUseWezen}}
npaddwezenes = [{{range .P2P.NPAddWezenes}}
"{{.}}", {{end}}
]
peerrole = "{{.P2P.PeerRole}}"

[wezen]
allowprivate = {{.Wezen.AllowPrivate}}
genesisfile = "{{.Wezen.GenesisFile}}"
enableblacklist = "{{.Wezen.EnableBlacklist}}"
