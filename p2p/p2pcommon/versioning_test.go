...
func TestParseMeyCoinVersion(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, err := ParseMeyCoinVersion(tt.args)
			if (err != nil) != tt.wantErr {
				t.Errorf("ParseMeyCoinVersion() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			if !tt.wantErr {
				if got.Major != int64(tt.wantM) || got.Minor != int64(tt.wantm) ||got.Patch != int64(tt.wantp) {
					t.Errorf("ParseMeyCoinVersion() got = %v, want %v.%v.%v-%v", got.String(), tt.wantM, tt.wantm, tt.wantp, tt.wantA)
				} else if len(tt.wantA) > 0 && got.PreRelease != semver.PreRelease(tt.wantA) {
					t.Errorf("ParseMeyCoinVersion() got = prerelase %v, want %v", got.PreRelease, tt.wantA)
				}
			}
		})
	}
...
func TestCurrentVersion(t *testing.T) {
	...
	_, err = ParseMeyCoinVersion(tagVer)
	if err != nil {
		t.Skipf("Skip test since not testable environment: %s", err.Error())
	}
	...
	if !CheckVersion(tagVer) {
		t.Fatalf("invalid version filtering setting. build version %v , want between %v (inclusive) and %v (exclusive)",
			tagVer, MinimumMeyCoinVersion, MaximumMeyCoinVersion)
	}
