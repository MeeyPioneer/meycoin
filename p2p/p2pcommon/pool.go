...
const (
	...
	WezenQueryInterval = time.Minute * 10
...
// It queries to Wezen or other connected peer efficiently.
// It determine if peer is
// NOTE that this object is not thread safe by itself.
type PeerFinder interface {
	PeerEventListener

	// Check if it need to discover more peers and send query request to wezen or other peers if needed.
	CheckAndFill()
}
...
type WaitingPeerManager interface {
	PeerEventListener
	// OnDiscoveredPeers is called when response of discover query came from wezen or other peer.
