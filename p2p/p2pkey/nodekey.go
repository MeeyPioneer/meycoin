...
func InitNodeInfo(baseCfg *config.BaseConfig, p2pCfg *config.P2PConfig, version string, logger *log.Logger) {
	...
	if !p2pcommon.CheckVersion(version) {
		logger.Warn().Str("minVersion",p2pcommon.MinimumMeyCoinVersion).Str("maxVersion",p2pcommon.MaximumMeyCoinVersion).Str("version",version).Msg("min/max version range is not set properly. change constant in source and then rebuild it")
	}
