...
import (
	"fmt"
	"github.com/meeypioneer/meycoin/p2p/list"
	"net"
	"sync"
	"time"

	"github.com/meeypioneer/meycoin/p2p/p2pkey"
	"github.com/meeypioneer/meycoin/p2p/raftsupport"
	"github.com/meeypioneer/meycoin/p2p/transport"
	...
	"github.com/meeypioneer/meycoin/p2p/subproto"
...
func (p2ps *P2P) Receive(context actor.Context) {
	...
	case *message.MapQueryRsp:
		if msg.Err != nil {
			p2ps.Logger.Info().Err(msg.Err).Msg("wezen returned error")
		} else {
			if len(msg.Peers) > 0 {
	...
	case message.GetRaftTransport:
		context.Respond(raftsupport.NewMeyCoinRaftTransport(p2ps.Logger, p2ps.nt, p2ps.pm, p2ps.mf, p2ps.consacc, msg.Cluster))
