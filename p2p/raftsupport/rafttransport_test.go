...
func TestMeyCoinRaftTransport_SendSnapshot(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			mockRA := p2pmock.NewMockMeyCoinRaftAccessor(ctrl)
			...
			target := NewMeyCoinRaftTransport(logger, mockNT, mockPM, mockMF, mockCA, dummyCl)
			target.snapF = ssf
...
func TestMeyCoinRaftTransport_NewSnapshotSender(t *testing.T) {
	logger := log.NewLogger("raft.support.test")
	dummyChainID := make([]byte, 32)
	dummyPeerID := types.RandomPeerID()
	...
	if _, ok := got.(*snapshotSender); !ok {
		t.Errorf("MeyCoinRaftTransport.NewSnapshotSender() type is differ: %v", reflect.TypeOf(got).Name())
	} else {
		if got.(*snapshotSender).peer != mockPeer {
			t.Errorf("MeyCoinRaftTransport.NewSnapshotSender() assign failed")
		}
	}
...
func TestMeyCoinRaftTransport_Send(t *testing.T) {
	// SendSnapshot acquire snap.Message, which must be closed after using.
