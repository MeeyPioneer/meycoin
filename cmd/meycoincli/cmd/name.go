...
func init() {
	...
	newCmd := &cobra.Command{
		Use:                   "new",
		Short:                 "Create account name. It spend at least an amount of meycoin according to nameprice",
		RunE:                  execNameNew,
	...
	newCmd.Flags().StringVar(&spending, "amount", "20meycoin", "spending for create name. Must be set to the current nameprice")
	...
	updateCmd.Flags().StringVar(&spending, "amount", "20meycoin", "spending for update name. Must be set to the current nameprice")
...
func execNameNew(cmd *cobra.Command, args []string) error {
	...
	tx := &types.Tx{
		Body: &types.TxBody{
			Account:   account,
			Recipient: []byte(types.MeyCoinName),
			Amount:    amount.Bytes(),
			Payload:   payload,
			GasLimit:  0,
