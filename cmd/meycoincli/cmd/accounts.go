...
func init() {
	accountCmd := &cobra.Command{
		...
		PersistentPreRun:  preConnectMeyCoin,
...
// This is so we can use meycoincli in a pipe and still read passwords from tty
...
func preConnectMeyCoin(cmd *cobra.Command, args []string) {
	if rootConfig.KeyStorePath == "" {
		connectMeyCoin(cmd, args)
	} else {
		client = nil
	}

	if len(sock) > 0 {
		if admClient = newMeyCoinAdminClient(sock); admClient == nil {
