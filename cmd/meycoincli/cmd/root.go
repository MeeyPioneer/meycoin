...
var (
	...
	rootCmd = &cobra.Command{
		Use:               "meycoincli",
		Short:             "MeyCoin light commandline interface",
		Long:              `MeyCoin is right`,
		PersistentPreRun:  connectMeyCoin,
		PersistentPostRun: disconnectMeyCoin,
	}
)
...
func init() {
	...
	rootCmd.PersistentFlags().StringVar(&svrName, "tlsservername", "", "meycoinsvr name for TLS ")
	rootCmd.PersistentFlags().StringVar(&cacrtFile, "tlscacert", "", "meycoinsvr CA certification file for TLS ")
	...
	rootCmd.PersistentFlags().Int32VarP(&port, "port", "p", 7845, "Port number to meycoin server")
	rootCmd.PersistentFlags().StringVar(&dataDir, "keystore", "$HOME/.meycoin", "Path to keystore")
...
func connectMeyCoin(cmd *cobra.Command, args []string) {
	if test {
		return
	}
...
func disconnectMeyCoin(cmd *cobra.Command, args []string) {
	if test {
		return
	}
