...
var enterpriseKeyCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		msg, err := client.GetEnterpriseConfig(context.Background(), &meycoinrpc.EnterpriseConfigKey{Key: args[0]})
		if err != nil {
			cmd.Printf("Failed: %s\n", err.Error())
			return
		}
...
func getConfChangeBlockNo(blockHash []byte) (meycoinrpc.BlockNo, error) {
	if len(blockHash) == 0 {
		return 0, fmt.Errorf("failed to get block since blockhash is empty")
	}

	block, err := client.GetBlock(context.Background(), &meycoinrpc.SingleBytes{Value: blockHash})
	if err != nil {
		return 0, err
	}
...
type OutConfChange struct {
	Payload  string
	TxStatus *meycoinrpc.EnterpriseTxStatus
}
...
func getChangeClusterStatus(cmd *cobra.Command, blockHash []byte, timer *time.Timer) (*meycoinrpc.ConfChangeProgress, error) {
	var (
		err               error
		cycle             = time.Duration(3) * time.Second
		msgConfChangeProg *meycoinrpc.ConfChangeProgress
	)
	...
	for {
		...
		msgConfChangeProg, err := client.GetConfChangeProgress(context.Background(), &meycoinrpc.SingleBytes{Value: b})
		if err != nil {
			continue
		}

		if msgConfChangeProg.State == meycoinrpc.ConfChangeState_CONF_CHANGE_STATE_APPLIED {
...
var enterpriseTxCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		var (
			tx         *meycoinrpc.Tx
			msgblock   *meycoinrpc.TxInBlock
			output     OutConfChange
			cycle      = time.Duration(3) * time.Second
			ci         types.CallInfo
			confChange *meycoinrpc.ConfChangeProgress
			timer      *time.Timer
		)
		...
		getTxTimeout := func() (*meycoinrpc.Tx, *meycoinrpc.TxInBlock, error) {
			for {
				tx, err = client.GetTX(context.Background(), &meycoinrpc.SingleBytes{Value: txHashDecode})
				if err == nil {
				...
				if err != nil {
					msgblock, err = client.GetBlockTX(context.Background(), &meycoinrpc.SingleBytes{Value: txHashDecode})
					if err != nil {
		...
		if msgblock != nil {
			receipt, err := client.GetReceipt(context.Background(), &meycoinrpc.SingleBytes{Value: txHashDecode})
			if err != nil {
				cmd.Println("Error: failed to get receipt")
				cmd.Println(output.ToString())
				return
			}
			output.TxStatus = &meycoinrpc.EnterpriseTxStatus{
