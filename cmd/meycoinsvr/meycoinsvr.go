...
var (
	rootCmd = &cobra.Command{
		Use:   "meycoinsvr",
		Short: "MeyCoin Server",
		Long:  "MeyCoin Server Full-node implementation",
...
func init() {
	...
	localFlags.BoolVar(&useTestnet, "testnet", false, "use MeyCoin TestNet; this only affects if there's no genesis block")
...
func initConfig() {
	...
	if cfg.EnableTestmode && cfg.UseTestnet {
		fmt.Println("Turn off test mode for MeyCoin Public Chains")
		os.Exit(1)
	}
}
...
func configureZipkin() {
	...
	if nil != collector {
		...
		tracer, err := zipkin.NewTracer(zipkin.NewRecorder(collector, false, myEndpoint, "meycoinsvr"))
		if err != nil {
...
func rootRun(cmd *cobra.Command, args []string) {
	...
	svrlog.Info().Str("revision", gitRevision).Str("branch", gitBranch).Msg("MEYCOIN SVR STARTED")
	...
	pmapSvc := wezenclient.NewWezenConnectSvc(cfg.P2P, p2pSvc)
