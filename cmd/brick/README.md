...
This is an interactive shell program to communicate with an meycoin vm for testing.
...
### MeyCoin VM for Testing

* provides an way to run smart contracts in same environment with meycoin
...
1. git clone and build meycoin in debug mode (release is ok, but you cannot debug contracts), unix & mac: `make debug`, windows + mingw64: `mingw32-make debug`
2. create a directory, where you want
3. copy binary `brick` to the directory, or add meycoin's `bin` folder to system PATH
...
creates an account and deposite meycoin. `inject inject <account_name> <amount>`
...
transfer meycoin between two addresses. `send <sender_name> <receiver_name> <amount>`

``` lua
1> send tester receiver 10
  INF send meycoin successfully cmd=send module=brick
```
...
``` lua
2> deploy tester 0 helloContract https://raw.githubusercontent.com/meeypioneer/mey-contract-ex/master/contracts/helloworld/test/test-helloworld.brick
  INF deploy a smart contract successfully cmd=deploy module=brick
```
...
``` lua
3> call tester 0 helloContract set_name `["meycoin"]`
...
14 call bj 0 helloctr set_name `["meycoin"]`
  INF call a smart contract successfully cmd=call module=brick
15
16 # query again, this now will print "hello meycoin"
17 query helloctr hello `[]` `"hello meycoin"`
  INF query compare successfully cmd=query module=brick
Batch is successfully finished
  INF batch exec is finished cmd=batch module=brick
```
