...
docker cp extract:/go/meycoin/bin/ .
docker cp extract:/go/meycoin/cmd/brick/arglog.toml bin/brick-arglog.toml
docker cp extract:/go/meycoin/libtool/lib/ .
docker rm -f extract

declare -a names=("node" "tools" "wezen")
for name in "${names[@]}"
do
    tagsExpanded=()
    for tag in "${tags[@]}"; do
        tagsExpanded+=("-t meycoin/$name:$tag")
    done
    echo "[meycoin/$name:${tags[*]}]"
...
do
    for tag in "${tags[@]}"
    do
        docker images meycoin/$name:$tag | tail -1
    done
done
...
do
    for tag in "${tags[@]}"
    do
        echo "  docker push meycoin/$name:$tag"
    done
done
