...
func TestName(t *testing.T) {
	...
	tx.Recipient = []byte(types.MeyCoinName)
...
func TestNameRecursive(t *testing.T) {
	...
	tx := &types.TxBody{Account: owner, Recipient: []byte(types.MeyCoinName), Payload: buildNamePayload(name1, types.NameCreate, "")}
...
func TestNameNil(t *testing.T) {
	...
	scs, err := sdb.GetStateDB().OpenContractStateAccount(types.ToAccountID([]byte("meycoin.system")))
