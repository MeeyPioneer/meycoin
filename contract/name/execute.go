...
func ExecuteNameTx(bs *state.BlockState, scs *state.ContractState, txBody *types.TxBody,
	...
	systemContractState, err := bs.StateDB.OpenContractStateAccount(types.ToAccountID([]byte(types.MeyCoinSystem)))
	...
	owner := getOwner(scs, []byte(types.MeyCoinName), false)
	if owner != nil {
