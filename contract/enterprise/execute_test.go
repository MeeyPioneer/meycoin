...
func TestEnterpriseChangeCluster(t *testing.T) {
	...
	tx.Payload = []byte(`{"name":"changeCluster", "args":[{"command" : "add", "name": "meycoinnew", "address": "/ip4/127.0.0.1/tcp/11001", "peerid":"16Uiu2HAmAAtqye6QQbeG9EZnrWJbGK8Xw74cZxpnGGEAZAB3zJ8B"}]}`)
	...
	bs = state.NewBlockState(&state.StateDB{})
	tx.Payload = []byte(`{"name":"changeCluster", "args":[{"command" : "nocmd", "name": "meycoinnew", "address": "/ip4/127.0.0.1/tcp/11001", "PeerID":"16Uiu2HAmAAtqye6QQbeG9EZnrWJbGK8Xw74cZxpnGGEAZAB3zJ8B"}]}`)
	_, err = ExecuteEnterpriseTx(bs, ccc, scs, tx, sender, receiver, testBlockNo)
	assert.Error(t, err)
	assert.Nil(t, bs.CCProposal)

	bs = state.NewBlockState(&state.StateDB{})
	tx.Payload = []byte(`{"name":"changeCluster", "args":[{"command" : "add", "name": "meycoinnew", "address": "http://127.0.0.1:1001", "peerid":"16Uiu2HAmAAtqye6QQbeG9EZnrWJbGK8Xw74cZxpnGGEAZAB3zJ8B"}]}`)
	_, err = ExecuteEnterpriseTx(bs, ccc, scs, tx, sender, receiver, testBlockNo)
	assert.Error(t, err)
	assert.Nil(t, bs.CCProposal)
}
