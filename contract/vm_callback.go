...
var (
	mulMeyCoin, mulGaer, zeroBig *big.Int
...
func init() {
	mulMeyCoin, _ = new(big.Int).SetString("1000000000000000000", 10)
...
func luaECVerify(L *LState, service C.int, msg *C.char, sig *C.char, addr *C.char) (C.int, *C.char) {
	...
	isMeyCoin := len(address) == types.EncodedAddressLength

	/*MeyCoin Address*/
	if isMeyCoin {
		bAddress, err := types.DecodeAddress(address)
		if err != nil {
			return -1, C.CString("[Contract.LuaEcVerify] invalid meycoin address: " + err.Error())
		}
...
func transformAmount(amountStr string) (*big.Int, error) {
	...
	r := regexp.MustCompile("(?i)meycoin|gaer|aer")
	...
	for _, pair := range res {
		...
		case 5:
			amountBig = new(big.Int).Mul(amountBig, mulMeyCoin)
		}
		if ret != nil {
...
func luaGovernance(L *LState, service C.int, gType C.char, arg *C.char) *C.char {
	...
	receiver := ctx.bs.InitAccountStateV([]byte(types.MeyCoinSystem), scsState.prevState, scsState.curState)
	txBody := types.TxBody{
