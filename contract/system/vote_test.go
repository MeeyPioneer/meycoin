...
func initTest(t *testing.T) (*state.ContractState, *state.V, *state.V) {
	...
	scs, err := bs.OpenContractStateAccount(types.ToAccountID([]byte("meycoin.system")))
	...
	receiver, err := bs.GetAccountStateV([]byte(types.MeyCoinSystem))
	assert.NoError(t, err, "could not get test address state")
...
func commitNextBlock(t *testing.T, scs *state.ContractState) *state.ContractState {
	...
	bs.Commit()
	cdb.UpdateRoot(bs)
	systemContractID := types.ToAccountID([]byte(types.MeyCoinSystem))
...
func TestBasicStakingVotingUnstaking(t *testing.T) {
	...
	sender.AddBalance(types.MaxMEY)
	tx := &types.Tx{
		Body: &types.TxBody{
			Account: sender.ID(),
	...
	assert.Equal(t, sender.Balance().Bytes(), new(big.Int).Sub(types.MaxMEY, types.StakingMinimum).Bytes(),
