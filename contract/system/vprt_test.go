...
func initVprtTestWithSc(t *testing.T, initTable func(*state.ContractState)) {
	...
	s, err := vprStateDB.OpenContractStateAccount(types.ToAccountID([]byte(types.MeyCoinSystem)))
...
func openSystemAccountWith(root []byte) *state.ContractState {
	s, err := vprChainStateDB.OpenNewStateDB(root).OpenContractStateAccount(types.ToAccountID([]byte(types.MeyCoinSystem)))
	if err != nil {
		return nil
	}
