...
func LoadDummyChain(opts ...func(d *DummyChain)) (*DummyChain, error) {
	...
	scs, err := bc.sdb.GetStateDB().OpenContractStateAccount(types.ToAccountID([]byte("meycoin.system")))
...
func (bc *DummyChain) GetStaking(name string) (*types.Staking, error) {
	scs, err := bc.sdb.GetStateDB().OpenContractStateAccount(types.ToAccountID([]byte(types.MeyCoinSystem)))
	if err != nil {
		return nil, err
	}
