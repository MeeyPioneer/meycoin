...
func TestContractSystem(t *testing.T) {
	...
	_ = bc.ConnectBlock(tx)
	receipt := bc.GetReceipt(tx.Hash())
	exRv := fmt.Sprintf(`["Amg6nZWXKB6YpNgBPv9atcjdm6hnFvs5wMdRgb2e9DmaF5g9muF2","5zv52oTnDQas6WWnftRRhtZiudNaNTJ72WZWfDjRtKCQ","AmhNNBNY7XFk4p5ym4CJf8nTcRTEHjWzAeXJfhP71244CjBCAQU3",%d,3,999]`, bc.cBlock.Header.Timestamp/1e9)
...
abi.register(get, checkEther, checkMeyCoin, keccak256)
`
	bc, err := LoadDummyChain()
	if err != nil {
		t.Errorf("failed to create test database: %v", err)
	...
	err = bc.Query("crypto", `{"Name": "checkMeyCoin", "Args" : []}`, "", `true`)
	if err != nil {
		t.Error(err)
	}
...
function constructor() 
    Symbol:set("TOKEN")
    Name:set("Standard Token on MeyCoin")
    Decimals:set(18)
...
`
	...
	bigSrc = `
function constructor()
    db.exec("create table if not exists meycoinjdbc001 (name text, yyyymmdd text)")
    db.exec("insert into meycoinjdbc001 values ('홍길동', '20191007')")
    db.exec("insert into meycoinjdbc001 values ('홍길동', '20191007')")
    db.exec("insert into meycoinjdbc001 values ('홍길동', '20191007')")
end

function inserts()
	db.exec("insert into meycoinjdbc001 select * from meycoinjdbc001")
end
