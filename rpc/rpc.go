...
type RPC struct {
	...
	*component.BaseComponent
	...
	actualServer  *MeyCoinRPCService
...
func (ns *RPC) BeforeStart() {
	meycoinrpc.RegisterMeyCoinRPCServiceServer(ns.grpcServer, ns.actualServer)
}
...
func (ns *RPC) Receive(context actor.Context) {
	...
	case []*types.Event:
		server := ns.actualServer
		for _, e := range msg {
			if bytes.Equal(e.GetContractAddress(), types.AddressPadding([]byte(types.MeyCoinEnterprise))) {
