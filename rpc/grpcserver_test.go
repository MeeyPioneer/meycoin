...
import (
	...
	"github.com/meeypioneer/meycoin/message/messagemock"
...
func TestMeyCoinRPCService_dummys(t *testing.T) {
...
func TestMeyCoinRPCService_GetTX(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			rpc := &MeyCoinRPCService{
				hub: tt.fields.hub, actorHelper: mockActorHelper, msgHelper: mockMsgHelper,
			}
			got, err := rpc.GetTX(tt.args.ctx, tt.args.in)
			if (err != nil) != tt.wantErr {
				t.Errorf("MeyCoinRPCService.GetTX() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			if !reflect.DeepEqual(got, tt.want) {
				t.Errorf("MeyCoinRPCService.GetTX() = %v, want %v", got, tt.want)
			}
		})
	}
}
