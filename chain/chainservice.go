...
func (core *Core) initGenesis(genesis *types.Genesis, mainnet bool, testmode bool) (*types.Block, error) {
	...
	if gen == nil {
		...
		if testmode {
			...
			if genesis == nil {
				if mainnet {
					//return nil, errors.New("to use mainnet, create genesis manually (visit http://docs.meycoin.io)")
...
func NewChainService(cfg *cfg.Config) *ChainService {
	...
	//reset parameter of meycoin.system
	systemState, err := cs.SDB().GetSystemAccountState()
	if err != nil {
		panic("failed to read meycoin.system state")
	}
...
func getAddressNameResolved(sdb *state.StateDB, account []byte) ([]byte, error) {
	if len(account) == types.NameLength {
		scs, err := sdb.OpenContractStateAccount(types.ToAccountID([]byte(types.MeyCoinName)))
		if err != nil {
